...
public:
	SentencePieceProcessor();
	SentencePieceProcessor(const std::string &path);
	...
	virtual util::Status NBestEncode(util::min_string_view input,
			int nbest_size,
	...
	virtual util::Status SampleEncode(util::min_string_view input,
...
#define DEFINE_SPP_DIRECT_FUNC_IMPL(FuncName, OutType, ...) \
  ...
  return output;
	...
	virtual std::vector<std::vector<std::string>> NBestEncodeAsPieces(
		...
		DEFINE_SPP_DIRECT_FUNC_IMPL(NBestEncode,
				std::vector<std::vector<std::string>>, input, nbest_size);
	}
	...
	virtual std::vector<std::string> SampleEncodeAsPieces(
		...
		DEFINE_SPP_DIRECT_FUNC_IMPL(SampleEncode, std::vector<std::string>,
...
#undef DEFINE_SPP_DIRECT_FUNC_IMPL
	...
	virtual util::bytes SampleEncodeAsSerializedProto(
			util::min_string_view input, int nbest_size, float alpha) const;

	virtual util::bytes NBestEncodeAsSerializedProto(
			util::min_string_view input, int nbest_size) const;
	...
	virtual std::vector<int> PieceToId(const std::vector<std::string> &ss) const;
...
private:
	...
	util::Status PopulateSentencePieceText(util::min_string_view input,
			util::min_string_view normalized,
