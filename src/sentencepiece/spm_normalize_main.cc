...
DEFINE_string(normalize_model, "", "Model file name");
DEFINE_bool(use_internal_normalization, false,
            "Use NormalizerSpec \"as-is\" to run the normalizer "
            "for SentencePiece segmentation");
DEFINE_string(normalize_rule_name, "",
              "Normalization rule name. "
              "Choose from nfkc or identity");
DEFINE_string(normalize_rule_tsv, "", "Normalization rule TSV file. ");
DEFINE_bool(delete_extra_whitespaces, true, "Remove extra whitespaces");
DEFINE_bool(decompile, false,
            "Decompile compiled charamap and output it as TSV.");
DEFINE_string(normalize_output, "", "Output filename");
...
int spm_normalize(int argc, char *argv[]) {
  std::vector<std::string> rest_args;
  sentencepiece::flags::ParseCommandLineFlags(argc, argv, &rest_args);

  NormalizerSpec spec;

  if (!FLAGS_normalize_model.empty()) {
    ModelProto model_proto;
    SentencePieceProcessor sp;
    CHECK_OK(sp.Load(FLAGS_normalize_model));
    spec = sp.model_proto().normalizer_spec();
  } else if (!FLAGS_normalize_rule_tsv.empty()) {
    spec.set_normalization_rule_tsv(FLAGS_normalize_rule_tsv);
    CHECK_OK(SentencePieceTrainer::PopulateNormalizerSpec(&spec));
  } else if (!FLAGS_normalize_rule_name.empty()) {
    spec.set_name(FLAGS_normalize_rule_name);
    CHECK_OK(SentencePieceTrainer::PopulateNormalizerSpec(&spec));
  } else {
    LOG(FATAL) << "Sets --model, normalize_rule_tsv, or "
                  "normalize_rule_name flag.";
  }
  ...
  if (!FLAGS_use_internal_normalization) {
    ...
    spec.set_remove_extra_whitespaces(FLAGS_delete_extra_whitespaces);
  }
  ...
  if (FLAGS_decompile) {
    ...
    CHECK_OK(Builder::SaveCharsMap(FLAGS_normalize_output, chars_map));
  } else {
    const Normalizer normalizer(spec);
    auto output = sentencepiece::filesystem::NewWritableFile(FLAGS_normalize_output);
