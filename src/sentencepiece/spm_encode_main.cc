...
DEFINE_string(encode_model, "", "encode_model file name");
DEFINE_string(
    encode_output_format, "piece",
    "choose from piece, id, proto, nbest_piece, nbest_id, or nbest_proto");
DEFINE_string(encode_output, "", "encode_output filename");
DEFINE_string(encode_extra_options, "",
...
int spm_encode(int argc, char *argv[]) {
  std::vector<std::string> rest_args;
  sentencepiece::flags::ParseCommandLineFlags(argc, argv, &rest_args);

  CHECK_OR_HELP(encode_model);

  sentencepiece::SentencePieceProcessor sp;
  CHECK_OK(sp.Load(FLAGS_encode_model));
  CHECK_OK(sp.SetEncodeExtraOptions(FLAGS_encode_extra_options));

  if (!FLAGS_vocabulary.empty()) {
    CHECK_OK(sp.LoadVocabulary(FLAGS_vocabulary, FLAGS_vocabulary_threshold));
  }

  auto output = sentencepiece::filesystem::NewWritableFile(FLAGS_encode_output);
  ...
  } else if (FLAGS_encode_output_format == "nbest_id") {
    process = [&](const std::string &line) {
  ...
  } else if (FLAGS_encode_output_format == "nbest_proto") {
    process = [&](const std::string &line) {
      CHECK_OK(sp.NBestEncode(line, FLAGS_nbest_size, &nbest_spt));
      //      output->WriteLine(nbest_spt.Utf8DebugString());
    };
  } else {
    LOG(FATAL) << "Unknown output format: " << FLAGS_encode_output_format;
  }
