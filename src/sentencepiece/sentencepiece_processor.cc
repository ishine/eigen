...
SentencePieceProcessor::SentencePieceProcessor(const std::string &path) {
	auto status = this->Load(path);
	if (!status.ok()) {
...
util::Status SentencePieceProcessor::Load(
	...
	for (const auto &s : model_proto_->self_test_data().samples()) {
		...
		if (s.expected() != result) {
			errors.emplace_back(
					string_util::StrCat(s.input(), "\t", s.expected(), "\t",
							result));
		}
	}
...
util::Status SentencePieceProcessor::SetVocabulary(
	...
	CHECK_OR_RETURN(type == TrainerSpec::UNIGRAM || type == TrainerSpec::BPE)<< "Vocabulary constraint is only enabled in subword units.";
...
util::Status SentencePieceProcessor::NBestEncode(util::min_string_view input,
		int nbest_size, std::vector<std::vector<std::string>> *pieces) const {
	CHECK_OR_RETURN_STATUS_STL(pieces);
...
util::Status SentencePieceProcessor::NBestEncode(util::min_string_view input,
		int nbest_size, std::vector<std::vector<int>> *ids) const {
	CHECK_OR_RETURN_STATUS_STL(ids);
...
util::Status SentencePieceProcessor::SampleEncode(util::min_string_view input,
		int nbest_size, float alpha, std::vector<std::string> *pieces) const {
	CHECK_OR_RETURN_STATUS_STL(pieces);
...
util::Status SentencePieceProcessor::SampleEncode(util::min_string_view input,
		int nbest_size, float alpha, std::vector<int> *ids) const {
	CHECK_OR_RETURN_STATUS_STL(ids);
...
util::Status SentencePieceProcessor::PopulateSentencePieceText(
	...
	for (const auto &p : result) {
		...
		if (IsControl(id)) {
			...
			const auto surface = absl::ClippedSubstr(input.data(), orig_begin,
					orig_end - orig_begin);
...
util::Status SentencePieceProcessor::Encode(util::min_string_view input,
	...
	RETURN_IF_ERROR(
			normalizer_->Normalize(string_util::ToSV(input), &normalized,
					&norm_to_orig));

	const auto result = model_->Encode(normalized);
	RETURN_IF_ERROR(
			PopulateSentencePieceText(input, normalized, norm_to_orig, result,
					spt));
...
util::Status SentencePieceProcessor::NBestEncode(util::min_string_view input,
		int nbest_size, NBestSentencePieceText *nbest_spt) const {
...
util::Status SentencePieceProcessor::SampleEncode(util::min_string_view input,
		int nbest_size, float alpha, SentencePieceText *spt) const {
	CHECK_OR_RETURN_STATUS_PROTO(spt);
...
util::bytes SentencePieceProcessor::SampleEncodeAsSerializedProto(
	...
	SentencePieceText spt;
	if (!SampleEncode(input, nbest_size, alpha, &spt).ok())
		return "";
	return spt.SerializeAsString();
}
...
util::bytes SentencePieceProcessor::NBestEncodeAsSerializedProto(
	...
	NBestSentencePieceText spt;
	if (!NBestEncode(input, nbest_size, &spt).ok())
		return "";
	return spt.SerializeAsString();
}
...
std::vector<int> SentencePieceProcessor::PieceToId(
		const std::vector<std::string> &s) const {
	int size = s.size();
...
util::Status SentencePieceProcessor::ApplyExtraOptions(
	...
	for (const auto &extra_option : extra_options) {
		...
		case EOS: {
			...
			piece->set_id(
					PieceToId(
							util::min_string_view(model_->eos_piece().data())));
		...
		case BOS: {
			...
			piece->set_id(
					PieceToId(
							util::min_string_view(model_->bos_piece().data())));
...
util::Status SentencePieceProcessor::ParseExtraOptions(
	...
	static std::map<absl::string_view, SentencePieceProcessor::ExtraOption> extra_option_map =
			{ { "bos", SentencePieceProcessor::BOS }, { "eos",
					SentencePieceProcessor::EOS }, { "reverse",
					SentencePieceProcessor::REVERSE } };
	for (const auto &s : string_util::SplitPiece(extra_option, ":")) {
		const auto it = extra_option_map.find(s);
		CHECK_OR_RETURN(it != extra_option_map.end())<< "option \"" << s << "\" is not available.";
		extra_options->push_back(it->second);

		if (it->second == SentencePieceProcessor::BOS) {
			CHECK_OR_RETURN(!IsUnknown(
							PieceToId(util::min_string_view(model_->bos_piece().data()))))<< "id for `" << model_->bos_piece() << "` is not defined.";
		}
		if (it->second == SentencePieceProcessor::EOS) {
			CHECK_OR_RETURN(!IsUnknown(
							PieceToId(util::min_string_view(model_->eos_piece().data()))))<< "id for `" << model_->eos_piece() << "` is not defined.";
		}
	}
	return util::OkStatus();
}
